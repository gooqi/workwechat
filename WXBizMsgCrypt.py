...
class Prpcrypt(object):
    ...
    def encrypt(self,text,corpid):
        ...
        """      
        # 16位随机字符串添加到明文开头
        #corpid = bytes.decode(corpid)
        #text = text.decode('utf-8')
        #text = self.get_random_str() + struct.pack("I",socket.htonl(len(text))) + text + corpid
        #text = bytes(self.get_random_str(),encoding='utf8')+struct.pack("I",socket.htonl(len(text))) + text.encode('utf-8')+  bytes(corpid,encoding='utf8')
        text = bytes(self.get_random_str(),encoding='utf8')+struct.pack("I",socket.htonl(len(text))) + text+  bytes(corpid,encoding='utf8')
        print('text')
        print(text)
    ...
    def decrypt(self,text,corpid):
        ...
        if  bytes.decode(from_corpid) != corpid:
            #print(bytes.decode(from_corpid))
            #print(corpid)
...
class WXBizMsgCrypt(object):
    ...
    def EncryptMsg(self, sReplyMsg, sNonce, timestamp = None):
        ...
        pc = Prpcrypt(self.key)
        sReplyMsg=sReplyMsg.encode('utf8')
